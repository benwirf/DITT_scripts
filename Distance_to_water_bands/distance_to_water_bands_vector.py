...
for pdk in pdk_lyr.getFeatures():
    ...
    pdk_feats = [ft for ft in output_lyr.getFeatures() if ft['Pdk Name'] == pdk_name]
    max_band_dist = max(ft['Outer dist'] for ft in output_lyr.getFeatures())
    max_band_dist_feat = [ft for ft in output_lyr.getFeatures() if ft['Outer dist'] = max_band_dist][0]
    all_vert_distances = []
    for wpt in pdk_wpts:
        for v in 
