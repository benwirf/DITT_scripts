        ...
        if dissolve_paddocks:
            ...
            for ft in lltpd_temp_result.getFeatures():
                if ft.geometry().intersects(all_pdks_projected):
                    ...
                    prop_area_atts = self.returnLandTypeAttributesForGeometry(prop_ix, all_pdks_projected, area_method)
        ...
        for pdk_ft in source_paddocks.getFeatures():
            ...
            for lt_ft in lltpd_temp_result.getFeatures():
                if lt_ft.geometry().intersects(pdk_geom):
                    ...
                    pdk_lt.atts.append([a for a in pdk_ft.attributes()])
                    pdk_area_atts = self.returnLandTypeAttributesForGeometry(pdk_ix, pdk_geom, area_method)
                    ...
                    pdk_lt_feat.setAttributes(prop_atts)
...
#        return results
    ...
    def returnLandTypeAttributesForGeometry(self, land_type_geom, pdk_geom, calc_method):
        if calc_method == 1:# Planar
            ...
            plt_area = da.measureArea(land_type_geom)
            ...
            plt_pcnt = (plt_area/da.measureArea(pdk_geom))*100
