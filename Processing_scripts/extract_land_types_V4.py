...
class ExtractLandTypes(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        output_spreadsheet_params = (QgsProcessingParameterString(self.OUTPUT_XLSX, 'Output spreadsheet parameters', optional=True))
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        output_spreadsheet_path = self.parameterAsString(parameters, self.OUTPUT_XLSX, context)
        
#        return {'output path': output_spreadsheet_path}
        ...
        if dissolve_paddocks:
            ...
            if property_land_types_temp.isValid():
                    ...
                    # Create an output layer and append to output_layers list (will be )
                    output_layers.append(QgsVectorLayer(plt_path, property_land_types_temp.name(), 'ogr'))
                    if load_outputs:
...
#                            model_feedback.pushDebugInfo(repr(pdk_5k_wa_land_types_temp.dataProvider().lastError()))
        ...
        return results # currently an empty dictionary
