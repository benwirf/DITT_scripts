...
#            model_feedback.pushInfo(repr(waterpoint_feats))
            ...
            result_raster = QgsProcessingUtils.getMapLayer(results[f'clipped_{paddock_name}'])
            max = result_raster.dataProvider().bandStatistics(1).maximumValue# in meters
            paddock_info.append(round(max/1000, 5))
        
        #Outside loop
        ####################Create CSV report###########################
        #Create a temporary layer, add fields [Paddock Name, Paddock Area ha, No. Waterpoints, Max Dist to Water]
        #add a feature for each paddock with attributes from info_map dict.
        # E.g. for pdk_name, atts_list in info_map.items():
            #atts = [pdk_name, atts_list[0], atts_list[1], atts_list[2], atts_list[3]]
            #feat.setAttributes(atts)
        #Then run export to spreadsheet alg
