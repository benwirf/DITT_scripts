...
#        self.action_remove.triggered.connect()
    ...
    def create_paddock_watered_areas(self):
        if not self.wa_lyr:
            ...
            self.wa_lyr = QgsVectorLayer(f'Polygon?crs={wa_crs.authid()}', '', 'memory')
        ...
        for row in range(self.tbl.rowCount()):
            ...
            pdk_id = int(pdk_info.split('('[1][0]))
            # get data from second cell of each row,
            # extract ids as integers, then- for each waterpoint id,
            # get wp feature, transform geometry to wa_crs, buffer, dissolve
            ...
            # Add layer to canvas layer set...
            
    def transformed_geom(self, g, src_crs, dest_crs):
        '''
        Transform a geometry between desired crs's
        '''
        pass
