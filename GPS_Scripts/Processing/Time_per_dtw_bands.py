...
class TimePerWateredBand(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        all_bands = dtw_band_lyr.uniqueValues(dtw_band_lyr.fields().lookupField(watered_band_field))
        
        all_datetimes = gps_lyr.uniqueValues(gps_lyr.fields().lookupField(datetime_field))
        all_dates = [dt.date() for dt in all_datetimes]
        ...
        for date in all_dates:
            ...
            t_range = t1.secsTo(tn)/3600# Write to Total time field
            ...
            for i in range(min_idx, max_idx):
                ...
                current_band = get_dtw_band_key(ft[dtw_fld])
                next_band = self.get_dtw_band_key(next_ft[dtw_fld])
            #    print(f'{ft.id()} - {current_band} - {next_band}')
            ...
            for k, v in dtw_band_times.items():
                ...
                checksum+=v/60
            atts.append(checksum)
            feat.setAttributes(atts)
            output_feats.append(feat)
        #######################################################
        temp_lyr.dataProvider().addFeatures(output_feats)
        # TODO: Add final feature with totals for all dates in collar period,
        # add to temp layer, then save temp layer to spreadsheet
        return {self.OUTPUT_XLSX: ''}
        
    def align_input_layers_crs(self, gps_collar_lyr, watered_band_lyr, transform_context):
        if not watered_band_lyr.crs().isGeographic() and gps_collar_lyr.crs() == watered_band_lyr.crs():
        ...
        elif watered_band_lyr.crs().isGeographic() and gps_collar_lyr.crs().isGeographic():
        ...
        if gps_collar_lyr.crs() != watered_band_lyr.crs():
            # We need to do some reprojection
            if not watered_band_lyr.crs().isGeographic():
                # Transform gps layer to match
                gps_layer = gps_collar_lyr.materialize(QgsFeatureRequest().setDestinationCrs(watered_band_lyr.crs(), transform_context))
                dtw_layer = watered_band_lyr.materialize(QgsFeatureRequest())
                return [gps_layer, dtw_layer]
            if not gps_collar_lyr.crs().isGeographic():
    ...
    def get_dtw_band_key(self, dtw):
        for k in dtw_band_times.keys():
