...
src_dir = r'C:\Users\qw2\Desktop\Ant_GPS_Collars\Anthonys_20240715\No_5_West'
src_file = 'No5_West_1159_20240715_132642.csv'
tgt_dir = r'C:\Users\qw2\Desktop\Ant_GPS_Collars\Anthonys_20240715\No_5_West'
...
def process_collar(input_dir, file_name, output_dir):
    ...
    buffered_pdk_lyr = QgsProject.instance().mapLayersByName('No_5_West_plus_100')[0]
    
    physical_pdk_lyr = QgsProject.instance().mapLayersByName('No5_West')[0]

    wpt_lyr = QgsProject.instance().mapLayersByName('ANT_waters_plus_waterholes')[0]
    #print(file_name)#e.g. No4_0236_20240715_115936.csv
    output_prefix = '_'.join(file_name.split("_")[:3])# Extra word 'West' bookended with underscores in file names
    collar_no = output_prefix.split('_')[2]# Extra word 'West' bookended with underscores in file names
    ...
    for fld in [fld for fld in src_flds[2:]]:
        new_flds.append(fld)
    #print(new_flds)
    temp_lyr = QgsVectorLayer('Point?crs=epsg:4326', '', 'memory')
