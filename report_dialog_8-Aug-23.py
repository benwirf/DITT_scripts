...
class ExportToPdfDialog(QDialog):
    def __init__(self):
        ...
        # self.print_button.clicked.connect(self.printReport)
    ...
    def setDefaultHtml(self):
        html_text = "<html>"
        html_text += "<body>"
        html_text += "<img src='file:///home/ben/MLA-PP/test_image/test_pdk_img.jpg' alt='Paddock Image' style='width: 100%; opacity: 0.2;'>"
        html_text += "</body>"
        html_text += "</html>"
...
#####**************METHODS TO GENERATE HTML CONTENT**********************#####
    def basicReportHtml(self, paddock_name, development_name):
        ...
        if pdks:
            pdk = pdks[0]
            pdk_geom = pdk.geometry()
            planned_pdks = [p for p in self.pdk_lyr.getFeatures() if p['Status'] == 'Planned' and p.geometry().intersection(pdk_geom).area() > 10]
            if planned_pdks:
                ...
                for pp in sorted(planned_pdks, key = lambda x: x['Name']):
                    ...
                    pp_3km_watered_area = self.plannedWateredArea(pp_geom, 3000)
                    all_3km_watered_areas.append(pp_3km_watered_area)
                    pp_3km_watered_pcnt = (pp_3km_watered_area/pp_area)*100
                    pp_5km_watered_area = self.plannedWateredArea(pp_geom, 5000)
...
##############################################################################
    ...
    def showPreview(self):
        ...
        #####ADVANCED REPORT TEMPLATE#####
            ...
            # print(f'Current: {current_layers_ordered}')
            ...
            # print(f'Future: {future_layers_ordered}')
            settings = QgsMapSettings()
            settings.setOutputDpi(350)
            ...
            advanced_html += "</html>"
            advanced_html += "<style>"
            advanced_html += "h1 {font-family: Arial, sans-serif;}"
            ...
            advanced_html += "table {margin-left: auto; margin-right: auto; margin-bottom: 15px;}"
            advanced_html += "body {background-color: #fcf5e1; text-align: center}"
            advanced_html += "</style>"
