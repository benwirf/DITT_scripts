...
fs_to_date = 'Jun_30'# Change Output folder path

fire_period = 'jan-jun-24'

# Used in extractbyexpression processing step
months_to_extract = '(1, 2, 3, 4, 5, 6)'
...
output_folder = r'C:\Users\qw2\Desktop\Pastoral_Properties_Burnt\Results_Jan-June-24'
...
firescars_jan_jun = processing.run("native:extractbyexpression",
                {'INPUT':firescars_on_pastoral_estate,
                'EXPRESSION':f'"month" IN {months_to_extract}',
                'OUTPUT':os.path.join(output_folder, f'Property_FS_Jan_1-{fs_to_date}.gpkg')})['OUTPUT']

print('dissolving result firescars...')
fs_jan_jun_dissolved = processing.runAndLoadResults("native:dissolve",
                {'INPUT':firescars_jan_jun,
                'FIELD':[],
                'OUTPUT':os.path.join(output_folder, f'fs_dissolved_{fire_period}.gpkg')})['OUTPUT']

fs_lyr = QgsVectorLayer(firescars_jan_jun, '', 'ogr')

fs_dissolved_lyr = QgsVectorLayer(fs_jan_jun_dissolved, '', 'ogr')
...
field_1_added = processing.run("native:fieldcalculator",
                ...
                'FIELD_LENGTH':0,
                'FIELD_PRECISION':3,
                'FORMULA':f'round("Property_FS_Jan_1-{fs_to_date}_area"/1000000, 3)',
                'OUTPUT':'TEMPORARY_OUTPUT'})['OUTPUT']
...
field_2_added = processing.runAndLoadResults("native:fieldcalculator",
                ...
                'FIELD_LENGTH':0,
                'FIELD_PRECISION':2,
                'FORMULA':f'round("Property_FS_Jan_1-{fs_to_date}_pc", 2)',
...
processing.run("native:exporttospreadsheet",
                ...
                'OUTPUT':os.path.join(output_folder, f'Property_Pcnt_Burnt_Jan_1-{fs_to_date}.xlsx'),
                'OVERWRITE':False})
